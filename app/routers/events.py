...
def create_event(payload: schemas.EventCreate, db: Session = Depends(get_db), _user=Depends(get_current_user)):
    event = models.Event(
...
def check_conflicts(db: Session = Depends(get_db), _user=Depends(get_current_user)):
...
def update_event(event_id: int, payload: schemas.EventUpdate, db: Session = Depends(get_db), _user=Depends(get_current_user)):
    event = db.query(models.Event).filter(models.Event.id == event_id).first()
    if not event:
        raise HTTPException(404, "Olay bulunamadı")
...
def delete_event(event_id: int, db: Session = Depends(get_db), _user=Depends(get_current_user)):
    event = db.query(models.Event).filter(models.Event.id == event_id).first()
    if not event:
        raise HTTPException(404, "Olay bulunamadı")
    db.delete(event)
